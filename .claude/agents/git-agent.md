...
- **Push branch format:** `feature/safe-backup_<N>_time_of_push`
  where `<N>` is a 1-based integer that increments on every push.
- **Examples:**
  - 1st push → `feature/safe-backup_1_time_of_push`
  - 2nd push → `feature/safe-backup_2_time_of_push`
  - 3rd push → `feature/safe-backup_3_time_of_push`
...
Parse every ref whose name matches `refs/heads/feature/safe-backup_(\d+)_time_of_push`,
...
be `feature/safe-backup_<N>_time_of_push`.
...
```bash
git checkout -b feature/safe-backup_<N>_time_of_push
```
...
```bash
git push -u origin feature/safe-backup_<N>_time_of_push
```
...
- **Base branch:** `feature/safe-backup`
- **Push branch:** `feature/safe-backup_<N>_time_of_push`
- **Remote:** origin
- **Commit:** <full SHA>
...
- Run `git fetch origin` locally and inspect
  `feature/safe-backup_<N>_time_of_push` before merging.
```
...
- The exact branch name that was pushed
  (`feature/safe-backup_<N>_time_of_push`).
- The remote URL.
- The commit SHA.
...
- **Never leave a stale local branch.** On failure paths, delete
  the local `feature/safe-backup_<N>_time_of_push` branch before
