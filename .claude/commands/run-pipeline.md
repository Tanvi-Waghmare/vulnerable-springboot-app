...
>    the edit and document it in *Residual Risks* — do not guess.
>
...
> **`.claude/reports/SECURE_REMEDIATION_REPORT.md`** with the
> per-finding schema and top-level sections defined in your agent file
> (Remediation Summary, Files Referenced, Vulnerability Remediations,
> Security Improvements, Residual Risks, Secure Coding Recommendations).
...
> not write outside `.claude/reports/`.
>
...
> findings, and the list of files you edited (empty if all Skipped).
...
- **Build status** from the remediation report (`Build verified:
  passed` or `Build verified: failed — all edits reverted`).
- Top-line counts from the remediation report (e.g. *12 findings: 9
  Applied, 3 Skipped — see Residual Risks* — broken down by severity).
...
- Reminder that both report files were overwritten on this run and
  are git-ignored, so nothing is staged.
